...
sections = [[1, 8, 8, 8, 10, 11, 11, 19, 19],
[1, 8, 9, 9, 10, 12, 11, 19, 18],
[1, 7, 7, 9, 9, 12, 12, 18, 18],
[1, 7, 3, 3, 9, 12, 13, 17, 17],
[1, 3, 3, 6, 6, 6, 13, 16, 16],
[1, 2, 3, 3, 3, 6, 13, 13, 16],
[1, 2, 2, 4, 5, 6, 14, 14, 15],
[1, 1, 4, 4, 5, 6, 14, 14, 15],
[1, 4, 4, 5, 5, 6, 6, 15, 15]]
...
l2l = {(l1,l2): m.binary_var() for l1 in lsizes for l2 in lsizes} #L for size l1 contains l2 numbers
...
for r in side:
    for c in side:
        ...
        for l1 in lsizes:
            for l2 in lsizes:
                m.add(z[(l2,r,c)]+l2l[(l1,l2)]-1 <= y[(l1,r,c)])
# l2l conditions:
for l1 in lsizes:
    m.add(m.sum(l2l[(l1,l2)] for l2 in lsizes) == 1)
    m.add(m.sum(l2l[(l2,l1)] for l2 in lsizes) == 1)
...
# Every connected region needs to have the same sum of numbers
q = 15
for i in range(1,20):
    m.add(m.sum(z[(l,r,c)]*l for r in side for c in side for l in lsizes if sections[r][c] == i) == q)
